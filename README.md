...
- **Audit log** (admin): *Audit log* shows who did what and when, filterable by action
  and bill number, with **Export CSV** (respects the active filter). Each **delete** row
  has a *Restore* button that re-creates the bill from the recorded values; each **edit**
  row has *Revert*, which puts the bill back to its previous values (both refuse if the
  bill number is now taken).
